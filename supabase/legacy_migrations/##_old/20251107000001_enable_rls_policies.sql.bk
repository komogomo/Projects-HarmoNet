...
-- Created: 2025-11-07
-- Based on: schema.prisma v1.7 (実際に投入されたスキーマ)
-- Tables: 31 tables
-- ==========================================
...
CREATE POLICY tenants_insert ON tenants
  FOR INSERT WITH CHECK (id::text = (auth.jwt() ->> 'tenant_id'));

CREATE POLICY tenants_update ON tenants
  FOR UPDATE USING (id::text = (auth.jwt() ->> 'tenant_id'));

CREATE POLICY tenants_delete ON tenants
  FOR DELETE USING (id::text = (auth.jwt() ->> 'tenant_id'));
...
CREATE POLICY role_inheritances_modify_admin_only ON role_inheritances
  FOR ALL USING ((auth.jwt() ->> 'role') = 'system_admin');
...
CREATE POLICY permissions_modify_admin_only ON permissions
  FOR ALL USING ((auth.jwt() ->> 'role') = 'system_admin');
...
CREATE POLICY role_permissions_modify_admin_only ON role_permissions
  FOR ALL USING ((auth.jwt() ->> 'role') = 'system_admin');
...
CREATE POLICY announcement_targets_select ON announcement_targets
  FOR SELECT USING (
    announcement_id IN (
      SELECT id FROM announcements WHERE tenant_id::text = (auth.jwt() ->> 'tenant_id')
    )
  );

CREATE POLICY announcement_targets_modify_admin ON announcement_targets
  FOR ALL USING (
    (auth.jwt() ->> 'role') IN ('system_admin', 'tenant_admin')
  );
